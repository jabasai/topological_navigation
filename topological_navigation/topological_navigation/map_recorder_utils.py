...
class MapRecorderCore:
    ...
    def add_node(self, pose, node_distance=0.0, force=False):
        ...
        if loop_node is not None:
            ...
            self._link(last_name, loop_node)
        ...
        self._link(last_name, name)

        self.tmap["meta"]["last_updated"] = _now()
        return name, True, f"node '{name}' added"

    def delete_last_node(self):
        """Remove the most-recently added node (and any edges pointing to it)."""
        if not self._order:
        ...
        self.tmap["meta"]["last_updated"] = _now()
        return True, f"removed node '{name}'"
